        ...
        stage('Clean'){
            steps{
                script{
                    ...
                    sh "docker ps | grep 'web_thetiptop' -v | awk -F ' ' '{ if(NR>1) print $1}' | xargs docker kill || true"
                    sh "docker ps -a | grep 'web_thetiptop' -v | awk -F ' ' '{ if(NR>1) print $1}' | xargs docker rm || true"
