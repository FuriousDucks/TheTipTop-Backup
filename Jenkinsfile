pipeline{
    ...
    stages{
        ...
        stage('Test'){
            steps{
                script{
                    sh 'docker exec -t ${CONTAINER_NAME} composer require --dev symfony/test-pack symfony/panther dbrekelmans/bdi vendor/bin/bdi detect drivers --no-interaction --no-progress'
